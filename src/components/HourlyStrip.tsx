import type { ResortWeather } from '../lib/weather';
import { weatherCodeIcon, LuSnowflake } from '../lib/icons';
  ...
  return (
    <div className="card hourly-card">
      ...
      <div className="hourly-strip">
        {slice.map((p, i) => {
          const wc = weatherCodeIcon(p.weatherCode);
          return (
            <div className={`hourly-item${i === 0 ? ' now' : ''}`} key={p.time.toISOString()}>
              <span className="hourly-time">{i === 0 ? '지금' : `${p.time.getHours()}시`}</span>
              <span className="hourly-emoji" aria-label={wc.label}><wc.Icon size={20} /></span>
              <span className="hourly-temp">{Math.round(p.temp)}°</span>
              {p.snowfall >= 0.1 && (
                <span className="hourly-snow"><LuSnowflake size={11} /> {p.snowfall.toFixed(1)}</span>
              )}
            </div>
          );
        })}
